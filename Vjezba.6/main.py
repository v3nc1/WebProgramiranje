...
@app.route("/postovi/comments/<post_id>")
def comments(post_id):
	...
	# Poziv REST servisa, HTTP GET
	response = requests.get(Config.commentsURL, params=parameters)
	
	# JSON decoder (http://docs.python-requests.org/en/latest/user/quickstart/#json-response-content)
	komentari = response.json()

	return render_template("comments.html", comments=komentari)
...
def zadaci():
	...
	response = requests.get(Config.todosURL, params=parameters)

	zadaci=response.json()

	return render_template("zadaci.html",todos=zadaci)
